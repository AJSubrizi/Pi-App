...
/// Full MCP server definition (from `pi mcp list --json` or inspect + config).
...
/// List configured MCP servers (CLI `pi mcp list --json` preferred).
...
pub fn list_mcp_server_defs(project_cwd: Option<&str>) -> Vec<McpServerDef> {
    ...
    // Prefer `pi mcp list --json` (full command/args/env/url).
...
/// Supports multi-line `args = [ ... ]` arrays used by Pi config.
...
/// Parse `pi mcp list --json` payload into server defs.
...
/// Persist a stdio MCP server into the agent PI_AGENT_HOME config (session_data_mode).
...
/// Parse `pi mcp doctor --json` output into a structured report.
