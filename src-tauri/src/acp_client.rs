...
//!
//! Local production sessions use Pi's documented RPC mode (`pi --mode rpc`)
//! over JSONL. The historical `AcpClient` name is kept internally for now so
//! the session manager and UI event surface can migrate without a flag day.
//! Remote TCP mode still accepts the legacy ACP protocol.
...
pub enum AcpEvent {
    ...
    /// Fire-and-forget surface requested by a Pi extension.
    ExtensionUi {
        method: String,
        payload: Value,
    },
...
pub struct AcpClient {
    ...
    /// ACP server over TCP instead of spawning `pi agent stdio`).
    ...
    /// Pi acknowledges `prompt` immediately, then streams until
    /// `agent_settled`. These waiters preserve the old host contract where
    /// `prompt()` resolves only after the complete turn has settled.
    pi_settled_waiters: ParkingMutex<Vec<oneshot::Sender<Result<(), String>>>>,
    pi_ui_requests: ParkingMutex<HashMap<u64, PiUiRequest>>,
}
...
impl AcpClient {
    ...
    /// Spawn Pi in its documented headless RPC mode.
    pub fn spawn_with_home(
        ...
        // GUI apps often inherit a sparse PATH; keep the absolute Pi path but
        // enrich PATH so nested tools (npx, node, git) resolve.
        let mut cmd = Command::new(&cli_path);
        cmd.args(["--mode", "rpc", "--approve"]);
        ...
        tracing::info!(
            "pi rpc: spawn cli={} cwd={} data_mode={}",
            cli_path.display(),
            cwd.display(),
            session_data_mode,
        );
        ...
        let mut child = cmd.spawn().map_err(|e| {
            AgentError::new(
                AgentErrorCode::CliNotFound,
                format!("failed to spawn pi --mode rpc: {e}"),
            )
        })?;
    ...
    /// instead of spawning `pi agent stdio`. The server speaks the exact
    ...
    fn fail_all_pending(&self, message: &str) {
        ...
        let waiters = self.pi_settled_waiters.lock().drain(..).collect::<Vec<_>>();
    ...
    async fn handle_line(self: Arc<Self>, line: &str) {
        ...
        if self.dialect == TransportDialect::PiRpc {
            self.handle_pi_message(msg).await;
            return;
        }
    ...
    async fn handle_pi_message(&self, msg: Value) {
            ...
            if let Some(id) = json_id_u64(msg.get("id")) {
                if let Some(p) = self.pending.lock().remove(&id) {
                    if msg.get("success").and_then(Value::as_bool).unwrap_or(false) {
                        let _ =
                            p.tx.send(Ok(msg.get("data").cloned().unwrap_or(Value::Null)));
                    } else {
                        let error = msg
                            .get("error")
        ...
        match event_type {
            "message_update" => {
                let delta = msg.get("assistantMessageEvent").unwrap_or(&Value::Null);
                let delta_type = delta.get("type").and_then(Value::as_str).unwrap_or("");
                let (kind, text) = match delta_type {
                    "text_delta" => (
                        Some(StreamKind::Assistant),
                ...
                if let Some(kind) = kind {
                    if !text.is_empty() {
                        let message_id = msg
                            .pointer("/message/id")
                            .or_else(|| msg.pointer("/assistantMessageEvent/partial/id"))
                            .and_then(Value::as_str)
            ...
            "tool_execution_start" | "tool_execution_update" | "tool_execution_end" => {
                ...
                let status = match event_type {
                    ...
                    _ if msg.get("isError").and_then(Value::as_bool).unwrap_or(false) => "failed",
                    _ => "completed",
                }
                .to_string();
                let title = pi_tool_title(&tool_name, msg.get("args"));
            ...
            "auto_retry_start" => {
                let _ = self.event_tx.send(AcpEvent::RetryState {
                    attempt: msg.get("attempt").and_then(Value::as_u64).unwrap_or(1) as u32,
                    max_retries: msg
                        .get("maxAttempts")
            ...
            "auto_retry_end" => {
                let _ = self.event_tx.send(AcpEvent::RetryState {
                    attempt: msg.get("attempt").and_then(Value::as_u64).unwrap_or(1) as u32,
                    max_retries: HOST_PROVIDER_MAX_RETRIES,
                    reason: msg
                        .get("finalError")
                        .and_then(Value::as_str)
                        .unwrap_or("")
                        .to_string(),
                    status: if msg.get("success").and_then(Value::as_bool).unwrap_or(false) {
                        "succeeded".into()
                    } else {
                        "failed".into()
                    },
            ...
            "compaction_end" => {
                let _ = self.event_tx.send(AcpEvent::ContextCompact {
                    trigger: msg
                        ...
                        .unwrap_or("auto")
                        .to_string(),
                    tokens_before: msg.pointer("/result/tokensBefore").and_then(Value::as_u64),
                    tokens_after: msg
            ...
            "extension_ui_request" => {
                ...
                if matches!(method.as_str(), "select" | "confirm" | "input" | "editor") {
                    let Some(pi_id) = msg.get("id").and_then(Value::as_str) else {
                        return;
                    };
                    let rpc_id = self.next_id.fetch_add(1, Ordering::SeqCst);
                    self.pi_ui_requests.lock().insert(
                        rpc_id,
                        PiUiRequest {
                            id: pi_id.to_string(),
                            method: method.clone(),
...
fn pi_model_key(model: Option<&Value>) -> Option<String> {
    let model = model?;
...
fn pi_tool_title(tool_name: &str, args: Option<&Value>) -> String {
    ...
    let detail = match tool_name {
        ...
        "read" | "write" | "edit" => args
            ...
            .and_then(Value::as_str),
        "grep" => args.get("pattern").and_then(Value::as_str),
        "find" | "ls" => args.get("path").and_then(Value::as_str),
        _ => None,
    };
...
fn pi_fire_and_forget_ui_event(method: &str, payload: Value) -> Option<AcpEvent> {
    if !matches!(
        method,
        "notify" | "setStatus" | "setWidget" | "setTitle" | "set_editor_text"
    ) {
...
impl AcpClient {
    ...
    async fn pi_request_timeout(
        ...
        if self.dialect != TransportDialect::PiRpc {
            return Err("Pi RPC command used with a non-Pi transport".into());
        }
        if !self.reader_alive.load(Ordering::SeqCst) {
            return Err(format!(
                "agent stdout closed before {command}; {}",
                self.format_exit_detail("process dead")
            ));
        }
        let id = self.next_id.fetch_add(1, Ordering::SeqCst);
        ...
        let mut msg = fields.as_object().cloned().unwrap_or_default();
        msg.insert("id".into(), Value::Number(id.into()));
        msg.insert("type".into(), Value::String(command.to_string()));
        let msg = Value::Object(msg);
        info!("pi rpc → {command} id={id}");
        if let Err(e) = self.write_line(&msg).await {
            self.pending.lock().remove(&id);
            return Err(format!("write {command} failed: {e}"));
        }
        match tokio::time::timeout(std::time::Duration::from_secs(timeout_secs), rx).await {
            Ok(Ok(Ok(v))) => Ok(v),
            Ok(Ok(Err(e))) => Err(e),
            Ok(Err(_)) => Err(format!("Pi RPC channel closed while waiting for {command}")),
            Err(_) => {
                self.pending.lock().remove(&id);
                Err(format!("Pi RPC timeout on {command} after {timeout_secs}s"))
            }
        }
    }
    ...
    /// Prefer `session/load` when `resume_session_id` is set (Pi persists agent
    /// sessions under PI_AGENT_HOME). Fall back to `session/new`.
    ...
    pub async fn initialize_and_open_session(
        ...
        if self.dialect == TransportDialect::PiRpc {
            return self.open_pi_session(resume_session_id).await;
        }
    ...
    /// Returns `(session_id, resumed)`.
    ///
    /// Injects **enabled** MCP servers (App Extensions prefs + `pi mcp list`)
    /// into `mcpServers` so independent PI_AGENT_HOME and shared mode both see tools.
    ...
    async fn open_pi_session(
        ...
        if let Some(path) = resume_session_id.map(str::trim).filter(|s| !s.is_empty()) {
            match self
                .pi_request_timeout(
                    "switch_session",
                ...
                .await
            {
                ...
                Ok(_) => {}
                Err(e) => {
                    warn!("pi switch_session soft-fail ({e}); starting a new session");
                    self.pi_request_timeout("new_session", json!({}), HANDSHAKE_TIMEOUT_SECS)
                        .await
                        .map_err(|e| self.map_handshake_err("new_session", e))?;
                }
            }
        } else {
            // A warm Pi process already owns the prior app session. Detach it
            // before opening a different empty app session.
            self.pi_request_timeout("new_session", json!({}), HANDSHAKE_TIMEOUT_SECS)
                .await
                .map_err(|e| self.map_handshake_err("new_session", e))?;
        }
        ...
        let state = self
            .pi_request_timeout("get_state", json!({}), HANDSHAKE_TIMEOUT_SECS)
            .await
            .map_err(|e| self.map_handshake_err("get_state", e))?;
        let sid = state
            .get("sessionFile")
            .and_then(Value::as_str)
            ...
            .ok_or_else(|| {
                AgentError::new(
                    AgentErrorCode::AgentCrashed,
                    "Pi get_state did not return a session file or id",
                )
            })?
            .to_string();
        *self.agent_session_id.lock() = Some(sid.clone());
        let model_id = pi_model_key(state.get("model"));
        let _ = self.event_tx.send(AcpEvent::State {
            backend: "pi_rpc".into(),
            agent_session_id: Some(sid.clone()),
            model_id,
    ...
    pub async fn set_model(&self, model_id: &str) -> Result<(), String> {
        ...
        if self.dialect == TransportDialect::PiRpc {
            if model_id == "auto" || model_id.starts_with("pi-") {
                return Ok(());
            }
            let (provider, id) = model_id
                .split_once('/')
                .ok_or_else(|| "Pi model ids must use provider/model format".to_string())?;
            let result = self
                .pi_request_timeout(
                    "set_model",
            ...
            let sid = self.agent_session_id.lock().clone();
            let _ = self.event_tx.send(AcpEvent::State {
                backend: "pi_rpc".into(),
                agent_session_id: sid,
                model_id: pi_model_key(Some(&result)).or_else(|| Some(model_id.to_string())),
            });
            return Ok(());
        }
        let sid = self
    ...
    pub async fn set_mode(&self, product_mode: &str) -> Result<String, String> {
        if self.dialect == TransportDialect::PiRpc {
            // Pi deliberately keeps plan/ask/permission flows out of core.
            // Packages and project extensions can add those commands without
            // the desktop shell pretending a built-in mode exists.
            return Ok(product_mode.to_string());
        }
        let sid = self
            .agent_session_id
            .lock()
    ...
    pub async fn prompt(&self, text: &str, images: &[(String, String)]) -> Result<(), AgentError> {
        if self.dialect == TransportDialect::PiRpc {
            self.stopped.store(false, Ordering::SeqCst);
            let (tx, rx) = oneshot::channel();
            self.pi_settled_waiters.lock().push(tx);
            if let Err(e) = self
                .pi_request_timeout(
                    "prompt",
                    wire_pi_prompt_params(text, images),
                    HANDSHAKE_TIMEOUT_SECS,
                )
                .await
            {
                self.pi_settled_waiters.lock().clear();
            ...
            {
                Ok(Ok(Ok(()))) => Ok(()),
                Ok(Ok(Err(e))) => Err(classify_rpc_error(&e)),
                Ok(Err(_)) => Err(AgentError::new(
                    AgentErrorCode::AgentCrashed,
                    "Pi turn waiter closed",
                )),
    ...
    pub async fn cancel(&self) -> Result<(), String> {
        if self.dialect == TransportDialect::PiRpc {
            ...
            self.pi_request_timeout("abort", json!({}), HANDSHAKE_TIMEOUT_SECS)
                .await?;
            return Ok(());
        }
        let sid = self
    ...
    pub async fn respond_ask_user_question(
        ...
        if self.dialect == TransportDialect::PiRpc {
            let request = self
                ...
                .lock()
                .remove(&rpc_id)
                .ok_or_else(|| "Pi extension UI request not found".to_string())?;
            let msg = match outcome {
                AskUserOutcome::Cancelled => json!({
                ...
                AskUserOutcome::Accepted { answers } => {
                    ...
                    if request.method == "confirm" {
                        let confirmed = matches!(
                            ...
                            "confirm" | "confirmed" | "yes" | "allow" | "ok"
                        );
                        json!({
                            "type": "extension_ui_response",
                            "id": request.id,
            ...
            return self.write_line(&msg).await;
        }
...
pub fn wire_initialize_params() -> Value {
    json!({
        "protocolVersion": 1,
        "clientInfo": { "name": "pi-app", "version": "0.1.0" },
        "capabilities": {}
    })
}
...
/// Host → Pi RPC `prompt` params, including native multimodal image content.
pub fn wire_pi_prompt_params(text: &str, images: &[(String, String)]) -> Value {
    let images = images
        .iter()
        .map(|(data, mime_type)| {
            json!({
...
#[cfg(test)]
mod pi_prompt_params_tests {
    use super::*;

    #[test]
    fn builds_native_pi_image_payload() {
        let params =
            wire_pi_prompt_params("Inspect this", &[("aGVsbG8=".into(), "image/png".into())]);
        assert_eq!(params["message"], "Inspect this");
        assert_eq!(params["images"][0]["type"], "image");
        assert_eq!(params["images"][0]["data"], "aGVsbG8=");
        assert_eq!(params["images"][0]["mimeType"], "image/png");
    }

    #[test]
    fn keeps_empty_images_array_for_text_only_prompts() {
        let params = wire_pi_prompt_params("Hello", &[]);
...
pub fn decode_session_update(params: &Value) -> Vec<AcpEvent> {
    ...
    match kind {
        ...
        // Pi/Gemini emit agent_thought_chunk; some paths also use "thought".
...
pub async fn probe_acp_server(addr: &str) -> AcpProbeResult {
    ...
    let req = r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1,"clientInfo":{"name":"pi-app-probe","version":"0"},"capabilities":{}}}"#;
    let write = async {
...
#[cfg(test)]
mod pi_extension_ui_tests {
    use super::*;

    #[test]
    fn forwards_all_documented_fire_and_forget_methods() {
        for method in [
            "notify",
            "setStatus",
            "setWidget",
            ...
            match pi_fire_and_forget_ui_event(method, payload.clone()) {
                Some(AcpEvent::ExtensionUi {
                    method: actual,
                ...
                other => panic!("unexpected event for {method}: {other:?}"),
            }
        }
    }

    #[test]
    fn rejects_dialog_and_unknown_methods_from_fire_and_forget_path() {
        assert!(pi_fire_and_forget_ui_event("select", json!({})).is_none());
        assert!(pi_fire_and_forget_ui_event("custom", json!({})).is_none());
    }
}
...
mod live_handshake_tests {
    ...
    #[tokio::test]
    async fn live_pi_rpc_session_under_30s() {
        if std::env::var("PI_APP_LIVE_RPC").ok().as_deref() != Some("1") {
            eprintln!("skip live Pi RPC (set PI_APP_LIVE_RPC=1)");
            return;
        }
