...
pub fn write_support_bundle(doctor_json: &str) -> Result<PathBuf, String> {
    ...
    let out = std::env::temp_dir().join(format!("pi-app-support-{stamp}.zip"));
...
pub fn write_session_bundle(
    ...
    let out = std::env::temp_dir().join(format!("pi-app-session-{short_id}-{stamp}.zip"));
...
- host/cli_probe.json — Pi CLI CLI discovery\n\
- logs/ — recent App log files (if present)\n\
- agent/ — Pi CLI session trail (events, history, terminal logs)\n\
\n\
...
pub fn reset_app_data(keep_secrets: bool) -> Result<serde_json::Value, String> {
    ...
    match crate::checkpoints::cleanup_all_refs() {
        Ok(ref_errors) => {
            for error in ref_errors {
                tracing::warn!("checkpoint ref cleanup during reset: {error}");
            }
        }
        Err(error) => tracing::warn!("checkpoint cleanup during reset: {error}"),
    }
...
mod tests {
    ...
    fn reset_keeps_secrets_when_requested() {
        ...
        let tmp = std::env::temp_dir().join(format!("pi-app-reset-test-{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(tmp.join("sessions")).unwrap();
        fs::write(tmp.join("sessions_index.json"), "[]").unwrap();
        fs::write(
            tmp.join("artifacts-v1.json"),
            r#"{"schemaVersion":1,"records":[]}"#,
        )
        .unwrap();
        fs::write(
            tmp.join("operations-v1.json"),
            r#"{"schemaVersion":1,"records":[]}"#,
        )
        .unwrap();
        fs::write(
            tmp.join("checkpoints-v1.json"),
            r#"{"schemaVersion":1,"records":[]}"#,
        )
        .unwrap();
        fs::write(
            tmp.join("repository-trust-v1.json"),
            r#"{"schemaVersion":1,"approvals":[]}"#,
        )
        .unwrap();
        fs::write(tmp.join("secrets.json"), r#"{"officialApiKey":"sk-test"}"#).unwrap();
        fs::write(tmp.join("settings.json"), "{}").unwrap();

        std::env::set_var("PI_APP_HOME", &tmp);
        ...
        assert!(!tmp.join("artifacts-v1.json").is_file());
        assert!(!tmp.join("operations-v1.json").is_file());
        assert!(!tmp.join("checkpoints-v1.json").is_file());
        assert!(!tmp.join("repository-trust-v1.json").is_file());
        assert!(tmp.join("sessions").is_dir()); // recreated empty
        std::env::remove_var("PI_APP_HOME");
        let _ = fs::remove_dir_all(&tmp);
    }
    ...
    fn support_bundle_creates_zip_without_secrets() {
        let _g = ENV_LOCK.lock().unwrap();
        let tmp = std::env::temp_dir().join(format!("pi-app-bundle-test-{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
