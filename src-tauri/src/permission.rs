...
pub enum PermissionPolicy {
    ...
    AllowOnce,
    AllowForSession,
    /// Pi CLI `dontAsk` — deny anything not pre-approved (no interactive prompt).
    DontAsk,
    /// Pi CLI `acceptEdits` — auto-approve file edit tools inside project.
    AcceptEdits,
    Deny,
...
/// Tools treated as file edits for `acceptEdits` mode (aligned with Pi CLI docs).
...
///
/// Rules (H05 + §17.3 + Pi CLI permission modes):
...
mod tests {
    ...
    fn ask_with_session_cache_auto_allows_in_project() {
        ...
        let mut c = SessionAllowCache::default();
        let root = std::env::temp_dir().join("pi-app-perm-ask-cache");
    ...
    fn ask_with_session_cache_never_outside() {
        let mut c = SessionAllowCache::default();
        let root = std::env::temp_dir().join("pi-app-perm-ask-out");
    ...
    fn ask_without_cache_does_not_auto() {
        ...
        let root = std::env::temp_dir().join("pi-app-perm-ask-empty");
        let _ = std::fs::create_dir_all(&root);
    ...
    fn accept_edits_auto_allows_edit_tools() {
        let c = SessionAllowCache::default();
        let root = std::env::temp_dir().join("pi-app-perm-accept");
        let _ = std::fs::create_dir_all(&root);
        let inside = root.join("f.txt");
        let _ = std::fs::write(&inside, "x");
        assert!(may_auto_allow(
    ...
    fn download_into_project_is_auto_allowed() {
        let c = SessionAllowCache::default();
        let root = std::env::temp_dir().join("pi-app-perm-dl");
    ...
    fn download_outside_project_not_auto_allowed() {
        let c = SessionAllowCache::default();
        let root = std::env::temp_dir().join("pi-app-perm-dl-out");
        let _ = std::fs::create_dir_all(&root);
    ...
    fn relative_traversal_is_outside_project() {
        let root = std::env::temp_dir().join("pi-app-perm-trav");
        let _ = std::fs::create_dir_all(&root);
    ...
    fn outside_project_never_auto_via_session_cache() {
        let mut c = SessionAllowCache::default();
        let root = std::env::temp_dir().join("pi-app-perm-proj");
        let _ = std::fs::create_dir_all(&root);
        let inside = root.join("src/a.rs");
