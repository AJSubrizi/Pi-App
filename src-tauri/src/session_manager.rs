//! Host session manager: real ACP default; mock only if PI_APP_ACP=mock.
//!
...
struct LiveSession {
    ...
    tools_this_turn: u32,
    /// Git checkpoint waiting for its after-turn capture.
...
/// Keep multimodal RPC payloads bounded. File references remain in the text
/// prompt when an image is unsupported, missing, or too large.
const PI_RPC_IMAGE_MAX_COUNT: usize = 8;
...
async fn load_pi_rpc_images(attachments: &[MessageAttachmentStored]) -> Vec<(String, String)> {
    let mut images = Vec::new();
    let mut seen = HashSet::new();
    let mut total_bytes = 0u64;

    for attachment in attachments {
        if images.len() >= PI_RPC_IMAGE_MAX_COUNT || attachment.is_dir {
            continue;
        }
        let path = Path::new(attachment.path.trim());
        let Some(mime_type) = pi_rpc_image_mime(path) else {
            continue;
        };
        ...
        let Ok(meta) = tokio::fs::metadata(path).await else {
            tracing::warn!("Pi image attachment unavailable: {}", path.display());
            continue;
        };
        if !meta.is_file()
            || meta.len() > PI_RPC_IMAGE_MAX_BYTES
            || total_bytes.saturating_add(meta.len()) > PI_RPC_IMAGE_MAX_TOTAL_BYTES
        {
            tracing::warn!(
                "Pi image attachment skipped due to size/type: {}",
                path.display()
            );
            continue;
        }
        let Ok(bytes) = tokio::fs::read(path).await else {
            tracing::warn!("Pi image attachment could not be read: {}", path.display());
            continue;
        };
        total_bytes = total_bytes.saturating_add(bytes.len() as u64);
        images.push((B64.encode(bytes), mime_type.to_string()));
    }
...
fn build_history_bootstrap(app_session_id: &str) -> Option<String> {
    ...
    let mut body = String::from(
        "[Prior conversation context: this chat continues an existing Pi App session. \
...
/// When user asks to open a Pi / foreign agent session by UUID, steer tools.
...
pub struct SessionManager {
    ...
    /// Serialize pre-turn checkpoint capture and prompt submission.
...
impl SessionManager {
    ...
    fn try_finish_deferred_prompt_complete(
        ...
        Self::schedule_active_checkpoint_after(s);
        tracing::info!("acp turn finished after deferred prompt_complete stop={stop_reason}");
        Some(empty)
    }

    fn schedule_active_checkpoint_after(s: &mut LiveSession) {
        if let Some(checkpoint_id) = s.active_checkpoint_id.take() {
            crate::checkpoints::schedule_capture_after(checkpoint_id);
        }
    }
    ...
    async fn connect_inner(
        ...
        let agent_model = prefs.model_id.clone();
        ...
        // Independent PI_AGENT_HOME: push permission into agent config before spawn so
            ...
            if !settings.remote_runtime.enabled && !probe.found {
                {
                    ...
                    if let Some(s) = guard.as_mut() {
                        let _ = s.fsm.connect_failed(AgentError::new(
                            ...
                            "Pi CLI not found. Install Pi or set its path in Settings.",
                        ));
                    }
                }
                let snap = self.snapshot();
            ...
            MockConnectMode::FailCliNotFound => {
                {
                    ...
                    if let Some(s) = guard.as_mut() {
                        let _ = s.fsm.connect_failed(AgentError::new(
                            AgentErrorCode::CliNotFound,
                            "Mock: CLI not found (PI_APP_ACP=mock demo)",
                        ));
                        s.backend = "mock_acp".into();
                    }
                }
    ...
    async fn handle_acp_event(self: &Arc<Self>, app: &AppHandle, process_id: &str, ev: AcpEvent) {
        ...
        match ev {
            ...
            AcpEvent::ExtensionUi { method, payload } => {
                let app_sid = self
                    .inner
                    .lock()
                    .as_ref()
                ...
                if let Some(obj) = body.as_object_mut() {
                    obj.insert("sessionId".into(), serde_json::Value::String(app_sid));
                    obj.insert("method".into(), serde_json::Value::String(method));
                }
                let _ = app.emit("session://extension_ui", body);
            }
            AcpEvent::RetryState {
                attempt,
                max_retries,
        ...
        match ev {
            ...
            AcpEvent::ExtensionUi { method, payload } => {
                ...
                if let Some(obj) = body.as_object_mut() {
                    obj.insert(
                        "sessionId".into(),
                        serde_json::Value::String(app_session_id.to_string()),
                    );
                    obj.insert("method".into(), serde_json::Value::String(method));
                }
                let _ = app.emit("session://extension_ui", body);
    ...
    ///
    /// Agent path: `x.ai/rewind/execute` (Pi CLI extension).
    ...
    pub async fn send_message(
        ...
        let checkpoint_context = {
            let guard = self.inner.lock();
            let s = guard.as_ref().ok_or("no active session")?;
            match (s.project_path.clone(), s.meta.project_id.clone()) {
                (Some(project_path), Some(project_id)) => Some((
                    project_path,
                    project_id,
                    s.app_session_id.clone(),
                    turn_id.clone(),
        ...
        let prepared_session_id = checkpoint_context
            .as_ref()
            .map(|(_, _, session_id, _)| session_id.clone());
        let mut active_checkpoint_id =
            if let Some((project_path, project_id, session_id, checkpoint_turn_id)) =
                checkpoint_context
            {
                match tokio::task::spawn_blocking(move || {
                    crate::checkpoints::capture_before_auto(
                        &project_path,
                        &project_id,
                        &session_id,
                        &checkpoint_turn_id,
                ...
                .await
                {
                    Ok(Ok(Some(checkpoint))) => Some(checkpoint.id),
                    Ok(Ok(None)) => None,
                    Ok(Err(error)) => {
                        tracing::warn!("pre-turn checkpoint capture failed: {error}");
                        None
                    }
                    Err(error) => {
                        tracing::warn!("pre-turn checkpoint task failed: {error}");
                        None
                    }
                }
            } else {
        ...
        let stored_attachments = attachments.unwrap_or_default();
        let prompt_images = load_pi_rpc_images(&stored_attachments).await;
        ...
        let (backend, app_sid, acp, agent_prompt) = {
            ...
            if prepared_session_id
                .as_ref()
                .is_some_and(|session_id| session_id != &s.app_session_id)
            {
                if let Some(checkpoint_id) = active_checkpoint_id.take() {
                    crate::checkpoints::schedule_capture_after(checkpoint_id);
                }
            ...
            s.tools_this_turn = 0;
            s.active_checkpoint_id = active_checkpoint_id;
            ...
            let _ = store::append_message(
                ...
                ChatMessageStored {
                    ...
                    attachments: (!stored_attachments.is_empty())
                        .then_some(stored_attachments.clone()),
                    marker: None,
                },
            );
            (
                ...
                move |chunk: StreamChunk| {
                    ...
                    if let Some(s) = guard.as_mut() {
                        ...
                        if chunk.done {
                            ...
                            SessionManager::schedule_active_checkpoint_after(s);
                        }
                    }
                    drop(guard);
                    if chunk.done {
        ...
        let app2 = app.clone();
        tokio::spawn(async move {
            if let Err(e) = acp.prompt(&agent_prompt, &prompt_images).await {
                {
                    let mut guard = mgr.inner.lock();
                    if let Some(s) = guard.as_mut() {
    ...
    /// the previous `PI_AGENT_HOME`. App session meta + journals stay; live shell is
    ...
    fn drain_all_agent_slots(&self) -> DrainedAgents {
        ...
        // Live
        {
            ...
            if let Some(s) = guard.as_mut() {
                ...
                s.fsm.soft_disconnect();
                s.process_id = String::new();
                // Old agent session lives under previous PI_AGENT_HOME — do not resume.
