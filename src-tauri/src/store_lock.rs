...
pub fn write_bytes_atomic(path: &Path, bytes: &[u8]) -> Result<(), String> {
    with_exclusive_lock(path, || write_bytes_atomic_under_lock(path, bytes))
}

/// Replace a file atomically when the caller already holds its exclusive lock.
///
/// Read-modify-write stores use this to keep the read and the replace inside
/// one lock acquisition. Calling [`write_bytes_atomic`] there would attempt to
/// acquire the same lock twice.
pub(crate) fn write_bytes_atomic_under_lock(path: &Path, bytes: &[u8]) -> Result<(), String> {
    if let Some(parent) = path.parent() {
