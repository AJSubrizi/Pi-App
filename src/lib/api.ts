 ...
 * @param attachments Local file cards persisted with the turn. Supported image
 *                    files are also sent through Pi RPC's native `images` field.
 */
...
/** Download + install latest Pi CLI (multi-mirror). Progress via setup://cli-install-progress. */
...
export type AppUpdateCheck = {
  ...
  /** Always Pi-App releases URL. */
  htmlUrl: string;
  /** Preferred installer download for this OS/arch when available. */
...
export function resolveAppUpdateOpenUrl(r: AppUpdateCheck): string {
  ...
  if (page && ok(page)) return page;
  return "https://github.com/AJSubrizi/Pi-App/releases";
}
...
/** Per-file added/removed line counts (Changes `+N` badges + header total). */
...
export async function gitNumstat(projectPath: string) {
  return invoke<GitNumstatResult>("git_numstat", { projectPath });
}

/** Result of a git write op (stage / commit / push). */
export interface GitOpResult {
  ok: boolean;
  output: string;
  reason?: string | null;
}
export async function gitStage(projectPath: string, paths: string[]) {
  return invoke<GitOpResult>("git_stage", { projectPath, paths });
}
export async function gitUnstage(projectPath: string, paths: string[]) {
  return invoke<GitOpResult>("git_unstage", { projectPath, paths });
}
...
export async function gitDiscard(
  ...
  return invoke<GitOpResult>("git_discard", {
    projectPath,
    tracked,
    untracked,
  });
...
/** CLI sessions under PI_AGENT_HOME (shared-mode discovery). */
...
/** Agent session folder under PI_AGENT_HOME (contains images/, etc.). */
...
export interface PiPackage {
  ...
  manifestStatus?: "valid" | "none" | "missing" | "invalid" | string;
  manifestError?: string | null;
  contributions?: PiPackageContributions;
}

export interface PiPackageContributions {
  commands: Array<{
    id: string;
    title: string;
  ...
  artifactKinds: Array<{ id: string; title: string }>;
  workflows: Array<{
    id: string;
    title: string;
    entry: string;
  ...
  hooks: Array<{ id: string; event: string; order: number }>;
  nativeCapabilities: Array<{ capability: string; reason: string }>;
  keybindings: Array<{
    command: string;
    keys: string;
    when?: string | null;
...
export interface PiPackagesResult {
  packages: PiPackage[];
  configDir: string;
}

export async function piPackagesList(projectPath?: string | null) {
  return invoke<PiPackagesResult>("pi_packages_list", {
    projectPath: projectPath ?? null,
  });
}

export async function piPackageInstall(opts: {
  source: string;
  local: boolean;
  projectPath?: string | null;
}) {
  return invoke<PiPackagesResult>("pi_package_install", {
    source: opts.source,
    local: opts.local,
    projectPath: opts.projectPath ?? null,
  });
...
export async function piPackageRemove(opts: {
  source: string;
  local: boolean;
  projectPath?: string | null;
}) {
  return invoke<PiPackagesResult>("pi_package_remove", {
    source: opts.source,
    local: opts.local,
    projectPath: opts.projectPath ?? null,
  });
...
export async function piPackagesUpdate(projectPath?: string | null) {
  return invoke<PiPackagesResult>("pi_packages_update", {
    projectPath: projectPath ?? null,
  });
}
...
export type ArtifactFormat = "markdown" | "html" | "json" | "text";
...
export async function artifactsList(filters?: {
  projectId?: string | null;
  sessionId?: string | null;
  kind?: ArtifactKind | null;
}) {
  return invoke<ArtifactRecord[]>("artifacts_list", {
...
export async function artifactGet(id: string) {
  return invoke<ArtifactRecord | null>("artifact_get", { id });
}

export async function artifactUpsert(input: ArtifactUpsertInput) {
  return invoke<ArtifactRecord>("artifact_upsert", { input });
}

/** Deletes index metadata only. The referenced project file is preserved. */
export async function artifactDelete(id: string, expectedRevision: number) {
  return invoke<void>("artifact_delete", { id, expectedRevision });
}
...
export async function operationBegin(input: {
  operationId: string;
  kind: string;
  projectId: string;
  sessionId?: string | null;
  ...
  return invoke<OperationRecord>("operation_begin", {
    input: { ...input, payload: input.payload ?? null },
  });
}

export async function operationTransition<Result = unknown>(input: {
  operationId: string;
  ...
  return invoke<OperationRecord<Result>>("operation_transition", {
    input: {
      ...input,
      result: input.result ?? null,
      recoverableError: input.recoverableError ?? null,
    },
  });
}

export async function operationGet<Result = unknown>(operationId: string) {
  return invoke<OperationRecord<Result> | null>("operation_get", {
    operationId,
  });
}

export async function operationsList(filters?: {
  projectId?: string | null;
  sessionId?: string | null;
  status?: OperationStatus | null;
  limit?: number;
}) {
  return invoke<OperationRecord[]>("operations_list", {
    projectId: filters?.projectId ?? null,
    sessionId: filters?.sessionId ?? null,
...
export interface CheckpointRevertResult {
  ...
  status: "completed" | "uncertain" | string;
  changedPaths: string[];
  recoverableError?: string | null;
}

export async function checkpointsList(filters?: {
  projectId?: string | null;
  sessionId?: string | null;
  limit?: number;
}) {
  return invoke<TurnCheckpoint[]>("checkpoints_list", {
    projectId: filters?.projectId ?? null,
    sessionId: filters?.sessionId ?? null,
    limit: filters?.limit ?? 100,
  });
...
export async function checkpointRevertPreview(checkpointId: string) {
  return invoke<CheckpointRevertPreview>("checkpoint_revert_preview", {
    checkpointId,
  });
}

export async function checkpointRevertApply(input: {
  checkpointId: string;
  expectedWorktreeDigest: string;
  operationId: string;
}) {
  return invoke<CheckpointRevertResult>("checkpoint_revert_apply", input);
}

export async function checkpointsGc(options?: {
  maxAgeDays?: number;
  maxRecords?: number;
}) {
  return invoke<{
...
export type RepositoryTrustStatus =
  | "missing"
  | "invalid"
  | "project-untrusted"
  | "untrusted"
...
export interface RepositoryContributionReview {
  kind: "mcp" | "skill" | "script" | string;
  id: string;
...
export interface RepositoryTrustReview {
  projectId: string;
  projectTrusted: boolean;
  configPath: string;
  status: RepositoryTrustStatus | string;
  digest?: string | null;
  shortDigest?: string | null;
  contributions: RepositoryContributionReview[];
  issues: string[];
  canApprove: boolean;
}

export async function repositoryTrustReview(projectId: string) {
  return invoke<RepositoryTrustReview>("repository_trust_review", {
    projectId,
  });
}

export async function repositoryTrustApprove(input: {
  projectId: string;
  expectedDigest: string;
  mode: "once" | "digest";
  operationId?: string | null;
}) {
  return invoke<RepositoryTrustReview>("repository_trust_approve", {
    input: { ...input, operationId: input.operationId ?? null },
  });
}

export async function repositoryTrustReject(input: {
  projectId: string;
  expectedDigest: string;
  operationId: string;
}) {
  return invoke<RepositoryTrustReview>("repository_trust_reject", { input });
}

export async function repositoryTrustRevoke(input: {
  projectId: string;
  operationId: string;
}) {
  return invoke<RepositoryTrustReview>("repository_trust_revoke", { input });
}
...
/**
 * Host envelope for `pi doctor --json` (see `parseCliDoctorEnvelope`).
...
export interface DoctorReport {
  ...
  /** Pi CLI CLI `doctor --json` envelope (optional for older hosts). */
...
/** Result of `pi doctor fix <id> --yes` (stdout/stderr already redacted). */
...
/**
 * Export Pi CLI CLI session trace via `pi trace <agentSessionId> --local`.
...
/** List skills via `pi inspect --json` (optional project cwd). */
...
/** List MCP servers via `pi inspect --json` (optional project cwd). */
...
// ── Project inspect summary (`pi inspect --json`, secret-safe DTO) ────────
...
// ── Plugins via `pi plugin …` ─────────────────────────────────────────────

/** Component counts from `pi inspect` plugins[].provides — Pi CLI shape. */
...
export interface PluginDto {
  ...
  status: string;
  /** Load state from Pi CLI config / enable|disable CLI. */
  enabled: boolean;
  repoKey?: string | null;
  /** Pi CLI scope: user / project / cli / marketplace name. */
  scope?: string | null;
  ...
  if (!isTauri()) {
    return {
      ...
      billing: {
        ...
        subscribeUrl: "https://grok.com/supergrok?referrer=pi-build",
        fetchedAt: null,
      },
      heatmap: [],
      callLogs: [],
...
/** Abort a running `pi login` (OAuth / device-code). No-op if none is running. */
...
export async function accountOpenSubscribe() {
  if (!isTauri()) {
    window.open(
      "https://grok.com/supergrok?referrer=pi-build",
      "_blank",
    );
    return;
  }
...
/** Switch to official Pi CLI or a custom provider (writes config.toml default). */
...
export async function projectRulesList(projectPath: string) {
  return invoke<ProjectRulesListResult | ProjectRuleEntry[]>(
    "project_rules_list",
    { path: projectPath },
  );
}
