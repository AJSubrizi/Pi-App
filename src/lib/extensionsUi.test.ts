...
describe("isCliMissingError", () => {
  it("detects host CLI missing message", () => {
    expect(isCliMissingError("Pi CLI CLI not found")).toBe(true);
  ...
  it("ignores other errors and empty", () => {
    ...
    expect(isCliMissingError("pi inspect timed out after 12s")).toBe(false);
    expect(isCliMissingError("Failed to parse pi inspect JSON")).toBe(false);
  });
});
...
describe("shortPathLabel", () => {
  ...
  it("truncates long paths keeping basename tail", () => {
    const long =
      "/Users/someone/Library/Application Support/com.grokapp.pi-app/agent-home/skills/my-skill/SKILL.md";
...
describe("mergeInspectErrors", () => {
  ...
  it("prefers CLI missing message", () => {
    expect(
      mergeInspectErrors("Pi CLI CLI not found", "timeout"),
    ).toBe("Pi CLI CLI not found");
    expect(
      mergeInspectErrors("timeout", "Pi CLI CLI not found"),
    ).toBe("Pi CLI CLI not found");
  });
  ...
  it("includes plugins error and prefers CLI missing across three", () => {
    ...
    expect(
      mergeInspectErrors("timeout", null, "Pi CLI CLI not found"),
    ).toBe("Pi CLI CLI not found");
  });
});
...
describe("plugin helpers", () => {
  ...
  it("builds plugin meta, provides, and row key like Pi CLI", () => {
    expect(
      pluginMetaLine({
        name: "demo",
        scope: "user",
  ...
  it("filters by load state (Pi CLI f key)", () => {
    const rows = [
      { name: "a", enabled: true },
