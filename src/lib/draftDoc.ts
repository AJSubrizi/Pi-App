 ...
 * Agent prompts serialize skills as `/name` (Pi CLI invocable form).
 */
...
/**
 * Detect an active `@mention` token at the end of `textBeforeCursor` — used to
 * open the object/file reference picker. `@` must sit at index 0 or right after
 * whitespace (so `user@host` and emails never trigger it).
 */
...
/** Live `@` token from a contenteditable element (what the user sees). */
