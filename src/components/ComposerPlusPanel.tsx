...
export function ComposerPlusPanel({
  ...
  /** Show the right-hand hint only while filtering — browse stays clean. */
  const showRight = q.length > 0;
    ...
    >
      <div className="composer-plus__search" aria-hidden>
        ...
        {q ? (
          ...
          <span className="composer-plus__search-ph">
            {tr("composer.plusSearch")}
          </span>
        )}
        {q ? (
          <>
            <span className="composer-plus__search-caret" aria-hidden />
            <span className="composer-plus__search-count">
              {entries.length}
            </span>
          </>
        ) : null}
      ...
      {rows.map((row, ri) => {
        if (row.type === "section") {
          // First group sits flush under the search bar; later groups are
          // split by a hairline — no uppercase labels, matching the
          // reference command palette.
          const hasEntryBefore = rows
            .slice(0, ri)
            .some((r) => r.type === "entry");
          return hasEntryBefore ? (
            <div
              key={row.id}
